...
class RuleGenerator:
    """
    Generates rules for MARC record linting based on predefined data.
    """
    ...
    def _read_rules(self) -> None:
        """Rebuild `_rules` from the large table at end of Lint.pm."""
        data = RULES_DATA.strip().split("\n\n")
        for block in data:
            ...
            if len(parts) < 2:
                continue
            tagno, repeatable, tag_desc = parts[0], parts[1], parts[2]
            self._parse_tag_rules(tagno, repeatable, tag_desc, lines[1:])

    def _parse_tag_rules(
        self, tagno: str, repeatable: str, tag_desc: str, lines: list[str]
    ) -> None:
        rules = self._rules.setdefault(tagno, {})
        rules["repeatable"] = True if repeatable == "R" else False
        rules["field_desc"] = tag_desc
        rules["indicators"] = {}
        rules["subfields"] = {}
        ...
        for line in lines:
            ...
            if key.startswith("ind"):
                ind_rules = rules["indicators"]
                # Description text, matching Perl's _nice_list behavior
                desc = "blank" if val == "blank" else self._nice_list(val)

                # Build regex that respects ranges like 0-9 and leading 'b' (blank)
                if val == "blank":
                    regex = r"^ $"
                else:
                    # Replace leading 'b' with space, leaving any ranges (e.g. 0-9) intact
                    raw = val
                    if raw.startswith("b"):
                        raw = " " + raw[1:]

                    # We want char-class *ranges* (like 0-9) to stay intact.
                    # So we scan `raw` and only escape characters that are
                    # genuinely unsafe, leaving '-' untouched when it appears
                    # between two non-special characters.
                    safe_chars: list[str] = []
                    for i, ch in enumerate(raw):
                        if ch == "-":
                            # Keep '-' as-is when it's a proper range separator
                            if 0 < i < len(raw) - 1:
                                safe_chars.append("-")
                            else:
                                safe_chars.append(r"\-")
                    ...
                    char_class = "".join(safe_chars)
                    regex = rf"^[{char_class}]$"

                ind_rules[key] = {"value": val, "description": desc, "regex": regex}
            else:
                rules["subfields"][key] = {
                    "repeatable": True if val == "R" else False,
                    "description": desc,
                }

    def _nice_list(self, s: str) -> str:
        m = re.search(r"(\d)-(\d)", s)
        if m:
            return s.replace(m.group(0), f"{m.group(1)} thru {m.group(2)}")
        digits = list(s)
        if digits and digits[0] == "b":
            digits[0] = "blank"
        if len(digits) <= 1:
            return "".join(digits)
        last = digits.pop()
        return ", ".join(digits) + f" or {last}"
